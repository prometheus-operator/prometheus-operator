          ...
          spec: {
            ...
            properties: {
              ...
              tracing: {
                ...
                properties: {
                  clientType: {
                    description: 'clientType defines the client used to export traces.',
                    type: 'string',
                  },
                  compression: {
                    description: 'compression defines the compression type used when sending traces.',
                    type: 'string',
                  },
                  endpoint: {
                    description: 'endpoint defines the remote endpoint where traces are sent.',
                    type: 'string',
                  },
                  headers: {
                    description: 'headers defines request headers associated with gRPC or HTTP requests.',
                    properties: {
                      headers: {
                        additionalProperties: {
                          properties: {
                            files: {
                              description: 'files defines the header values loaded from files.',
                              items: {
                                type: 'string',
                              },
                              type: 'array',
                            ...
                            secrets: {
                              description: 'secrets defines the header values loaded from secrets.',
                              items: {
                                type: 'string',
                              },
                              type: 'array',
                            ...
                            values: {
                              description: 'values defines the literal header values.',
                              items: {
                                type: 'string',
                              },
                              type: 'array',
                        ...
                        description: 'headers defines a map of header names to their values.',
                        type: 'object',
                      },
                    },
                    type: 'object',
                  ...
                  insecure: {
                    description: 'insecure defines whether to disable client transport security.',
                    type: 'boolean',
                  },
                  samplingFraction: {
                    anyOf: [
                    ...
                    description: 'samplingFraction defines the probability that a trace will be sampled.',
                    pattern: '^(\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))(([KMGTPE]i)|[numkMGTPE]|([eE](\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))))?$',
                    'x-kubernetes-int-or-string': true,
                  },
                  timeout: {
                    description: 'timeout defines the maximum time the exporter waits for each batch export.',
                    pattern: '^(0|(([0-9]+)y)?(([0-9]+)w)?(([0-9]+)d)?(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?)$',
                    type: 'string',
                  },
                  tlsConfig: {
                    description: 'tlsConfig defines the TLS configuration used by the tracing client.',
                    properties: {
                      ca: {
                        description: 'ca defines the Certificate authority used when verifying server certificates.',
                        properties: {

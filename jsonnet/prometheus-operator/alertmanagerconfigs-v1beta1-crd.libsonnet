                  ...
                  properties: {
                    ...
                    incidentioConfigs: {
                      ...
                      items: {
                        ...
                        properties: {
                          alertSourceToken: {
                            description: "alertSourceToken defines the secret's key that contains the Incident.io alert source token.\nThis token authenticates requests against the Incident.io API.",
                            properties: {
                              key: {
                                description: 'key defines the key of the secret to select from.  Must be a valid secret key.',
                                minLength: 1,
                          ...
                          httpConfig: {
                            description: 'httpConfig defines the HTTP client configuration for Incident.io webhook requests.',
                            properties: {
                              authorization: {
                                description: 'authorization defines the authorization header configuration for the client.\nThis is mutually exclusive with BasicAuth and is only available starting from Alertmanager v0.22+.',
                                properties: {
                          ...
                          maxAlerts: {
                            description: 'maxAlerts defines the maximum number of alerts to be sent per Incident.io request.\nWhen 0, all alerts are included in the request payload.',
                            format: 'int32',
                            minimum: 0,
                            type: 'integer',
                          },
                          ...
                          timeout: {
                            description: 'timeout defines the maximum time to wait for an Incident.io request to complete,\nbefore failing the request and allowing it to be retried.\nIt requires Alertmanager >= v0.29.0.',
                            pattern: '^(0|(([0-9]+)y)?(([0-9]+)w)?(([0-9]+)d)?(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?)$',
                            type: 'string',
                          },
                          url: {
                            description: 'url defines the Incident.io webhook URL to send HTTP POST requests to.\nurlSecret takes precedence over url. One of urlSecret and url should be defined.',
                            type: 'string',
                          },
                          urlSecret: {
                            description: "urlSecret defines the secret's key that contains the Incident.io webhook URL.\nurlSecret takes precedence over url. One of urlSecret and url should be defined.\nThe secret needs to be in the same namespace as the AlertmanagerConfig\nobject and accessible by the Prometheus Operator.",
                            properties: {
                              key: {
                                description: 'key defines the key of the secret to select from.  Must be a valid secret key.',
                                minLength: 1,

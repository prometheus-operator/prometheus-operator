...
func testPrometheusRuleStatusSubresourceForThanosRuler(t *testing.T) {
	...
	p, err = framework.CreateThanosRulerAndWaitUntilReady(ctx, ns, p)
	require.NoError(t, err)
	...
	// Record the lastTransitionTime value.
	pr1, err = framework.WaitForRuleCondition(ctx, pr1, p, monitoringv1.ThanosRulerName, monitoringv1.Accepted, monitoringv1.ConditionTrue, 3*time.Minute)
	require.NoError(t, err)

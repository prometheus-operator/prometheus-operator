...
| Field | Description | Scheme | Required |
| ----- | ----------- | ------ | -------- |
| metadata | Standard object’s metadata. More info: https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#metadata | [metav1.ObjectMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.11/#objectmeta-v1-meta) | false |
...
| listenAddress | Specify to which address Alertmanager will bind to. Note this is only for the Alertmanager UI, not the gossip communication. This option takes precedence over ListenLocal. | string | false |
| clusterListenLocal | ListenLocal makes the Alertmanager cluster listen on loopback, so that it does not bind against the Pod IP. Note this is only for the Alertmanager cluster (gossip communication), not the ui. | bool | false |
| clusterListenAddress | Specify to which address Alertmanager will bind to. Note this is only for the Alertmanager cluster (gossip communication), not the ui. This option takes precedence over ListenLocal. | string | false |
...
| listenAddress | ListenAddress makes the Prometheus server listen on the provided address instead of binding against the Pod IP. This option takes precedence over ListenLocal. | string | false |
...
| listenAddress | ListenAddress makes the Thanos sidecar listen on the provided address instead of binding against the Pod IP. This option takes precedence over ListenLocal. | string | false |

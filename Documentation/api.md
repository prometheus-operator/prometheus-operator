...
| status | Most recent observed status of the Prometheus cluster. Read-only. More info: https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#spec-and-status | [PrometheusStatus](#prometheusstatus) | false |
...
| tracingConfig | TracingConfig configures tracing in Prometheus. This is an experimental feature, it may change in any upcoming release in a breaking way. | *[PrometheusTracingConfig](#prometheustracingconfig) | false |
...
| Field | Description | Scheme | Required |
| ----- | ----------- | ------ | -------- |
| clientType | Client used to export the traces. Options are 'http' or 'grpc'. | string | true |
| endpoint | Endpoint to send the traces to. Should be provided in format <host>:<port>. | string | true |
| samplingFraction | Sets the probability a given trace will be sampled. Must be a float from 0 through 1. | string | true |
| insecure | If disabled, the client will use a secure connection. | *bool | true |
| headers | Key-value pairs to be used as headers associated with gRPC or HTTP requests. | map[string]string | true |
| compression | Compression key for supported compression types. Supported compression: gzip | string | true |
| timeout | Maximum time the exporter will wait for each batch export. Default '10s' | Duration | true |
| tlsConfig | TLS Config to use when sending traces. | *[TLSConfig](#tlsconfig) | true |
...
<em>appears in: [APIServerConfig](#apiserverconfig), [AlertmanagerEndpoints](#alertmanagerendpoints), [Endpoint](#endpoint), [PrometheusTracingConfig](#prometheustracingconfig), [RemoteReadSpec](#remotereadspec), [RemoteWriteSpec](#remotewritespec), [ThanosRulerSpec](#thanosrulerspec), [ThanosSpec](#thanosspec)</em>

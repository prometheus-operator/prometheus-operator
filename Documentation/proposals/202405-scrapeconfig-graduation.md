...
Currently, the Prometheus Operator supports 15 Service Discovery configurations out of the 28 offered by Prometheus. The limited number of supported
Service Discoveries may restrict users who rely on other Service Discovery mechanisms. Additionally, certain fields present in Prometheus
`scrape_config` are missing from the ScrapeConfig CRD, potentially hindering the configuration flexibility and completeness that we originally strived for.
...
- Pave the way for graduating the ScrapeConfig CRD to Beta.
...
### Enhancing Service Discovery Support
...
- **`dockerswarm_sd_config`**: Docker Swarm is a popular container orchestration tool. Supporting Docker Swarm Service Discovery will cater to users leveraging Docker Swarm for their deployments.
- **`puppetdb_sd_config`**: PuppetDB is a storage service that keeps data generated by Puppet. Integrating PuppetDB Service Discovery will be beneficial for users who utilize Puppet for configuration management.
- **`lightsail_sd_config`**: Amazon Lightsail is a simplified cloud platform. Including Lightsail Service Discovery will assist users who prefer Lightsail for its ease of use and cost-effectiveness.

The following is a list of Service Discoveries we will not be supporting yet:
*Note: this list is subject to change and the Service Discoveries can be added on respective feature requests. Users
are encouraged to file a feature request if they see a service discovery they use in the list.*
...
#### Filling Gaps in The CRD

Ensuring that the ScrapeConfig CRD accurately reflects Prometheus's `scrape_config` is essential for inching
closer to a 1:1 relationship with the Prometheus `scrape_config`. This involves including all relevant fields and
ensuring they are correctly implemented.
...
- Missing `MinLenght` validation on multiple string fields.
...
We have a bunch of options for the graduation strategy:
1. We can declare that we don’t automate `v1alpha1` -> `v1beta1` and any breaking change needs to
2. We can make no API changes in `v1beta1` -> in that case, a conversion webhook isn’t required.
3. We can introduce a `v1alpha2` with all breaking changes but no conversion and say that it will become `v1beta1`.
4. We implement `v1alpha1` -> `v1beta1` conversion with a conversion webhook.
...
To maintain high standards of quality and reliability, we will:
...
- **Explore End-to-End Testing**: Investigate the use of [testcontainers](https://golang.testcontainers.org/) to set up comprehensive end-to-end testing environments. This will help verify the validity and robustness of our Service Discovery configurations in real-world scenarios. However, this is not a priority.

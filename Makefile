...
test-e2e: test/instrumented-sample-app/certs/cert.pem test/instrumented-sample-app/certs/key.pem
	go test -timeout 120m -v ./test/e2e/ $(TEST_RUN_ARGS) --kubeconfig=$(KUBECONFIG) --operator-image=$(IMAGE_OPERATOR):$(TAG) --reloader-image=$(IMAGE_RELOADER):$(TAG) --webhook-image=$(IMAGE_WEBHOOK):$(TAG) --image-pull-secret=$(IMAGE_PULL_SECRET) -count=1

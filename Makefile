...
generate-crds: $(CONTROLLER_GEN_BINARY) $(GOJSONTOYAML_BINARY) $(TYPES_V1_TARGET) $(TYPES_V1ALPHA1_TARGET) $(TYPES_V1BETA1_TARGET)
	...
	find example/prometheus-operator-crd/ -name '*.yaml' ! -name 'kustomization.yaml' -print0 | xargs -0 -I{} sh -c '$(GOJSONTOYAML_BINARY) -yamltojson < "$$1" | jq > "$(PWD)/jsonnet/prometheus-operator/$$(basename $$1 | cut -d'_' -f2 | cut -d. -f1)-crd.json"' -- {}
	echo "# Code generated using 'make generate-crds'. DO NOT EDIT" > $(PWD)/example/prometheus-operator-crd/kustomization.yaml
	$(GOJSONTOYAML_BINARY) <<< "{apiVersion: kustomize.config.k8s.io/v1beta1, kind: Kustomization, resources: [$$(find example/prometheus-operator-crd/ -name '*.yaml' ! -name 'kustomization.yaml' | cut -d'/' -f4 | tr '\n' ',')]}" >> example/prometheus-operator-crd/kustomization.yaml
	...
	$(GOJSONTOYAML_BINARY) <<< "{apiVersion: kustomize.config.k8s.io/v1beta1, kind: Kustomization, resources: [$$(find example/prometheus-operator-crd-full/ -name '*.yaml' ! -name 'kustomization.yaml' | cut -d'/' -f4 | tr '\n' ',')]}" >> example/prometheus-operator-crd-full/kustomization.yaml
...
jsonnet-fmt: $(JSONNETFMT_BINARY)
	# jsonnet will match *.jsonnet and *.libsonnet files but nothing else in this repository

...
func newConfigBuilder(logger *slog.Logger, amVersion semver.Version, store *assets.Store, matcherStrategy monitoringv1.AlertmanagerConfigMatcherStrategy) *configBuilder {
	cg := &configBuilder{
		logger:    logger,
		amVersion: amVersion,
		store:     store,
...
func (tc *tlsConfig) sanitize(amVersion semver.Version, logger *slog.Lo) error {
	if tc == nil {
		return nil
	}

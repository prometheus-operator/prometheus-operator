...
func TestStatefulSetVolumes(t *testing.T) {
	...
	}, defaultTestConfig, []string{"rules-configmap-one"}, "", nil)
	require.NoError(t, err)
...
func TestSidecarResources(t *testing.T) {
	operator.TestSidecarsResources(t, func(reloaderConfig operator.ContainerConfig) *appsv1.StatefulSet {
		...
		sset, err := makeStatefulSet(tr, testConfig, []string{"my-configmap"}, "", nil)
		require.NoError(t, err)
		return sset
	})
}
...
func TestStatefulSetMinReadySeconds(t *testing.T) {
	...
	statefulSet, err := makeStatefulSetSpec(&tr, defaultTestConfig, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStatefulSetServiceName(t *testing.T) {
	...
	spec, err := makeStatefulSetSpec(&tr, defaultTestConfig, nil, nil)
	if err != nil {
		t.Fatal(err)
	}

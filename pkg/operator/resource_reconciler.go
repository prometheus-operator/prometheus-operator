...
func (rr *ResourceReconciler) processNextReconcileItem(ctx context.Context) bool {
	...
	if err == nil {
		...
		// Update the last reconcile time on successful reconciliation
...
// updateLastReconcileTime records the current time as the last reconcile time for the given key.
func (rr *ResourceReconciler) updateLastReconcileTime(key string) {
	rr.lastReconcile[key] = time.Now()
}

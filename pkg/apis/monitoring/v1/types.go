...
type PrometheusSpec struct {
	...
	// AdditionalRelabelConfigs allows specifying a key of a Secret containing
	// additional Prometheus relabel configs that get appended to the relabel config
	// of every scrape target. Relabel configurations specified must have the form as
	// specified in the official Prometheus documentation:
	// https://prometheus.io/docs/prometheus/latest/configuration/configuration/#relabel_config.
	// As relabel configs are appended, the user is responsible to make sure it
	// is valid. Note that using this feature may expose the possibility to
	// break upgrades of Prometheus. It is advised to review Prometheus release
	// notes to ensure that no incompatible scrape configs are going to break
	// Prometheus after the upgrade.
	AdditionalRelabelConfigs *v1.SecretKeySelector `json:"additionalRelabelConfigs,omitempty"`
	// AdditionalMetricRelabelConfigs allows specifying a key of a Secret containing
	// additional Prometheus metric relabel configs that get appended to the metric relabel config
	// of every scrape target. Relabel configurations specified must have the form as
	// specified in the official Prometheus documentation:
	// https://prometheus.io/docs/prometheus/latest/configuration/configuration/#relabel_config.
	// As relabel configs are appended, the user is responsible to make sure it
	...
	// notes to ensure that no incompatible scrape configs are going to break
	// Prometheus after the upgrade.
	AdditionalMetricRelabelConfigs *v1.SecretKeySelector `json:"additionalMetricRelabelConfigs,omitempty"`

...
// ValidateSafeTLS semantically validates the safe TLS options of WebTLSConfig.
func (c *WebTLSConfig) ValidateSafeTLS() error {
	if c.ClientCA != (SecretOrConfigMap{}) {
		if err := c.ClientCA.Validate(); err != nil {
			return fmt.Errorf("clientCA %s: %w", c.ClientCA.String(), err)
		}
	}
	...
	if c.Cert != (SecretOrConfigMap{}) && c.KeySecret == (v1.SecretKeySelector{}) {
		return fmt.Errorf("client cert specified without client key")
	}

	if c.KeySecret != (v1.SecretKeySelector{}) && c.Cert == (SecretOrConfigMap{}) {
...
func (c *WebTLSConfig) Validate() error {
	...
	if c.ClientCA != (SecretOrConfigMap{}) {
		if c.ClientCAFile != "" {
			msg := fmt.Sprintf("cannot specify both clientCAFile and ca")

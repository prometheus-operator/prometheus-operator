...
type CommonPrometheusFields struct {
	...
	// enableRemoteWriteReceiver enables Prometheus to be used as a receiver for the Prometheus remote
	// write protocol.
	//
	...
	// enableOTLPReceiver enables Prometheus to be used as a receiver for the OTLP Metrics protocol.
	//
	...
	// nameEscapingScheme specifies the character escaping scheme that will be requested when scraping
	...
	// hostAliases is the optional list of hosts and IPs that will be injected into the Pod's
	...
	// walCompression configures compression of the write-ahead log (WAL) using Snappy.
	//
	...
	// runtime configures the values for the Prometheus process behavior
	// +optional
...
type PrometheusSpec struct {
	...
	// retention is how long to retain the Prometheus data.
	//
	// Default: "24h" if `spec.retention` and `spec.retentionSize` are empty.
	// +optional
	Retention Duration `json:"retention,omitempty"`
	// retentionSize is the maximum number of bytes used by the Prometheus data.
	// +optional
...
type PrometheusTracingConfig struct {
	...
	// samplingFraction sets the probability a given trace will be sampled. Must be a float from 0 through 1.
	// +optional
	...
	// headers is the key-value pairs to be used as headers associated with gRPC or HTTP requests.
	// +optional
	...
	// timeout is the maximum time the exporter will wait for each batch export.
	// +optional
...
type StorageSpec struct {
	// disableMountSubPath eprecated: subPath usage will be removed in a future release.
	// +optional
...
type ThanosSpec struct {
	...
	// grpcServerTlsConfig configures the TLS parameters for the gRPC server providing the StoreAPI.
	//
...
type RemoteReadSpec struct {
	...
	// +optional
	BasicAuth *BasicAuth `json:"basicAuth,omitempty"`
	// bearerTokenFile is the file from which to read the bearer token for the URL.
	//
	// Deprecated: this will be removed in a future release. Prefer using `authorization`.
	// +optional
	BearerTokenFile string `json:"bearerTokenFile,omitempty"`
...
type ShardStatus struct {
	// shardID is the identifier of the shard.
	// +required
...
type TSDBSpec struct {
	// outOfOrderTimeWindow configures how old an out-of-order/out-of-bounds sample can be with
...
type SafeAuthorization struct {
	...
	// credentials selects a key of a Secret in the namespace that contains the credentials for authentication.
	// +optional
...
type OTLPConfig struct {
	...
	// translationStrategy configures how the OTLP receiver endpoint translates the incoming metrics.
	//
	// It requires Prometheus >= v3.0.0.
	// +optional
	...
	// convertHistogramsToNHCB configures optional translation of OTLP explicit bucket histograms into native histograms with custom buckets.
	// It requires Prometheus >= v3.4.0.
	// +optional

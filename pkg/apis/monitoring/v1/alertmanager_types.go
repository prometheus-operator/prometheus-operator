...
type Alertmanager struct {
	...
	// spec is the specification of the desired behavior of the Alertmanager cluster. More info:
	// https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
	// +required
	Spec AlertmanagerSpec `json:"spec"`
	// status is the most recent observed status of the Alertmanager cluster. Read-only.
	// More info:
	// https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
	// +optional
...
type AlertmanagerSpec struct {
	...
	// replicas is the expected size of the alertmanager cluster. The controller will
	// eventually make the size of the running cluster equal to the expected
	// size.
	// +optional
	Replicas *int32 `json:"replicas,omitempty"`
	// retention is the time duration Alertmanager shall retain data for. Default is '120h',
	...
	// persistentVolumeClaimRetentionPolicy is the field controls if and how PVCs are deleted during the lifecycle of a StatefulSet.
	...
	// resources  defines requests and limits for single Pods.
	// +optional
	Resources v1.ResourceRequirements `json:"resources,omitempty"`
	// affinity if specified, the pod's scheduling constraints.
	// +optional
	Affinity *v1.Affinity `json:"affinity,omitempty"`
	// tolerations ff specified, the pod's tolerations.
	// +optional
	Tolerations []v1.Toleration `json:"tolerations,omitempty"`
	// topologySpreadConstraints if specified, the pod's topology spread constraints.
	// +optional
	...
	// serviceName is the service name used by the underlying StatefulSet(s) as the governing service.
	...
	// clusterGossipInterval is the interval between gossip attempts.
	// +optional
	...
	// clusterPushpullInterval is the interval between pushpull attempts.
	// +optional
	ClusterPushpullInterval GoDuration `json:"clusterPushpullInterval,omitempty"`
	// clusterPeerTimeout is the timeout for cluster peering.
	// +optional
	ClusterPeerTimeout GoDuration `json:"clusterPeerTimeout,omitempty"`
	// portName for the pods and governing service.
	// Defaults to `web`.
	...
	// alertmanagerConfigSelector to be selected for to merge and configure Alertmanager with.
	// +optional
	...
	// web efines the web command line flags when starting Alertmanager.
	// +optional
	...
	// clusterTLS configures the mutual TLS configuration for the Alertmanager cluster's gossip protocol.
	//
	...
	// enableFeatures enable access to Alertmanager feature flags. By default, no features are enabled.
...
type AlertmanagerConfigMatcherStrategy struct {
	// type AlertmanagerConfigMatcherStrategyType defines the strategy used by
...
type AlertmanagerConfiguration struct {
	// name of the AlertmanagerConfig resource which is used to generate the Alertmanager configuration.
...
type AlertmanagerGlobalConfig struct {
	// smtp configures global SMTP parameters.
	// +optional
...
type AlertmanagerStatus struct {
	...
	// replicas is the total number of non-terminated pods targeted by this Alertmanager
	// object (their labels match the selector).
	// +required
	Replicas int32 `json:"replicas"`
	// updatedReplicas is the total number of non-terminated pods targeted by this Alertmanager
	...
	// unavailableReplicas is the total number of unavailable pods targeted by this Alertmanager object.
	// +required
	UnavailableReplicas int32 `json:"unavailableReplicas"`
	// selector used to match the pods targeted by this Alertmanager object.
	// +optional
	Selector string `json:"selector,omitempty"`
	// conditions is the current state of the Alertmanager object.
	// +listType=map
	// +listMapKey=type
	// +optional
...
type AlertmanagerWebSpec struct {
	...
	// getConcurrency is the maximum number of GET requests processed concurrently. This corresponds to the
...
type AlertmanagerLimitsSpec struct {
	// maxSilences is the maximum number active and pending silences. This corresponds to the
	...
	// maxPerSilenceBytes is the maximum size of an individual silence as stored on disk. This corresponds to the Alertmanager's
...
type GlobalSMTPConfig struct {
	// from is the default SMTP From header field.
	// +optional
	From *string `json:"from,omitempty"`

	// smartHost is the default SMTP smarthost used for sending emails.
	// +optional
	SmartHost *HostPort `json:"smartHost,omitempty"`

	// hello is the default hostname to identify to the SMTP server.
	// +optional
	...
	// requireTLS is the default SMTP TLS requirement.
	// Note that Go does not support unencrypted connections to remote SMTP endpoints.
	// +optional
	RequireTLS *bool `json:"requireTLS,omitempty"`

	// tlsConfig is the default TLS configuration for SMTP receivers
	// +optional
...
type GlobalTelegramConfig struct {
	// apiURL is he default Telegram API URL.
	//
...
type GlobalJiraConfig struct {
	// apiURL is the default Jira API URL.
	//
	// It requires Alertmanager >= v0.28.0.
	//
	// +optional
...
type GlobalRocketChatConfig struct {
	// apiURL is the default Rocket Chat API URL.
	//
	// It requires Alertmanager >= v0.28.0.
	//
	// +optional
	...
	// token is the default Rocket Chat token.
	//
	// It requires Alertmanager >= v0.28.0.
	//
	// +optional
	...
	// tokenID is the default Rocket Chat Token ID.
	//
	// It requires Alertmanager >= v0.28.0.
	//
	// +optional
...
type GlobalWebexConfig struct {
	// apiURL is the is the default Webex API URL.
	//
...
type GlobalWeChatConfig struct {
	// apiURL is he default WeChat API URL.
	// The default value is "https://qyapi.weixin.qq.com/cgi-bin/"
	// +optional
	APIURL *URL `json:"apiURL,omitempty"`

	// apiSecret is the default WeChat API Secret.
	// +optional
	APISecret *v1.SecretKeySelector `json:"apiSecret,omitempty"`

	// apiCorpID is the default WeChat API Corporate ID.
	// +optional
	// +kubebuilder:validation:MinLength=1
...
type GlobalVictorOpsConfig struct {
	// apiURL is the default VictorOps API URL.
	//
	// +optional
	APIURL *URL `json:"apiURL,omitempty"`
	// apiKey is the default VictorOps API Key.
	//
	// +optional
...
type HTTPConfig struct {
	...
	// bearerTokenSecret is the secret's key that contains the bearer token to be used by the client
	// for authentication.
	// The secret needs to be in the same namespace as the Alertmanager
	// object and accessible by the Prometheus Operator.
	// +optional
	BearerTokenSecret *v1.SecretKeySelector `json:"bearerTokenSecret,omitempty"`
	// tlsConfig for the client.
	// +optional
	TLSConfig *SafeTLSConfig `json:"tlsConfig,omitempty"`

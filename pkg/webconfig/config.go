...
func (c Config) generateConfigFileContents(ctx context.Context, secretClient typedcorev1.SecretInterface) ([]byte, error) {
	var err error
	if c.tlsConfig == nil && c.httpConfig == nil && c.basicAuthUsers == nil {
		return []byte{}, nil
	}
...
func (c Config) addBasicAuthUsersConfigToYaml(ctx context.Context, cfg yaml.MapSlice, secretClient typedcorev1.SecretInterface) (yaml.MapSlice, error) {
	...
	basicAuthUsers, err := k8s.GetSecret(ctx, secretClient, c.basicAuthUsers.SecretRef.Name)
	if err != nil {
		return cfg, err
	}

	for k, v := range basicAuthUsers.Data {
		if _, err := bcrypt.Cost(v); err != nil {
			return cfg, fmt.Errorf("hased value of user:%s is failed to bcrypt password: %w", k, err)
		}

		yamlData = append(yamlData, yaml.MapItem{Key: k, Value: string(v)})
	}

	return append(cfg, yaml.MapItem{Key: "basic_auth_users", Value: yamlData}), nil
}

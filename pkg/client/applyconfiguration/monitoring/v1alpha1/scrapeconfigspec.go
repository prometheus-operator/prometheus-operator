...
type ScrapeConfigSpecApplyConfiguration struct {
	...
	OutscaleSDConfigs []OutscaleSDConfigApplyConfiguration `json:"outscaleSDConfigs,omitempty"`
...
// WithOutscaleSDConfigs adds the given value to the OutscaleSDConfigs field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the OutscaleSDConfigs field.
func (b *ScrapeConfigSpecApplyConfiguration) WithOutscaleSDConfigs(values ...*OutscaleSDConfigApplyConfiguration) *ScrapeConfigSpecApplyConfiguration {
	for i := range values {
		if values[i] == nil {
			panic("nil value passed to WithOutscaleSDConfigs")
		}
		b.OutscaleSDConfigs = append(b.OutscaleSDConfigs, *values[i])
	}
	return b
}

...
func TestScrapeConfigSpecConfigWithDockerSDConfig(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			golden: "ScrapeConfigSpecConfig_DockerSD_with_TLSConfig.golden",
		},
		{
			name: "docker_sd_config_host_networking_host",
			scSpec: monitoringv1alpha1.ScrapeConfigSpec{
				DockerSDConfigs: []monitoringv1alpha1.DockerSDConfig{
					{
						Host: "hostAddress",
			...
			golden: "ScrapeConfigSpecConfig_DockerSD_HostNetworkingHostConfig.golden",
		}} {
		t.Run(tc.name, func(t *testing.T) {
			c := fake.NewSimpleClientset(
				&v1.Secret{
